 ...
 * mechanisms
 * Eureka server service urls 有两种配置的机制
 * 1.注册DNS上的信息；2.在配置中指定
 * 1) By registering the information in the DNS. 2) By specifying it in the
 * configuration.
 * </p>
 *
 * 一旦客户被注册，用户可以使用虚拟主机名（VIPAddress）来查找信息
...
public interface EurekaClientConfig {
     ...
     */
    //初始化实例信息到Eureka服务端的间隔时间，单位：秒 todo 这个参数不是很懂
     ...
     */
    //如果服务器支持的话，是否压缩从服务器上得到的信息
    //现在已经被禁用，因为以后的版本会强制压缩
     ...
     * </p>
     * 当server列表是从dns获取得到时，获取与server通信的url 上下文信息 todo 不知道是怎么用的
     ...
     *         listening.
     */
    //当server列表是从dns获取得到时，获取与server通信的端口信息 todo 不知道是怎么用的
     ...
     *         servers.
     */
    //当server列表是从dns获取得到时，获取服务器对应的dns名 todo 不知道是怎么用的
     ...
     * on client shutdown.
     * 当进程关闭时，是否向 Eureka-Server 显式注销自身服务
     *
     ...
     */
    //是否打印增量日志 todo 是否打印增量和全量差异。默认值 ：false 。从目前代码实现上来看，暂时没有生效。注意 ：开启该参数会导致每次增量获取后又发起全量获取，不要开启
     ...
     *
     * 获取 Eureka-Client 所在区域( myZone )的 Eureka-Server 服务地址
     ...
     * 客户端是否只对单一VIP感兴趣,只获得一个 `vipAddress` 对应的应用实例们的注册信息。
     ...
     */
    //当初始化注册不成功的时候是否会抛出异常，todo 有一个问题，如果设置为false，何时重试
